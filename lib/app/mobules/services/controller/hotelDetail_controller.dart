...
class HotelDetailController extends GetxController {
  ...
  var modal = HotelDetailModelCalass().obs;
  var isEmptyData = false.obs;
  // List<TourAppModel> modelDataList=List<TourAppModel>.empty().obs;

  List<HotelDetailModelCalass> flightDetailmodelClass =
      <HotelDetailModelCalass>[].obs;
...
// original data
  fatchProductList() async {
    ...
    try {
      isLoading(true);
      var product = await HotelDetailApiClientProvider().getFlightDetailData();

      if (product is HotelDetailModelCalass) {
        print("data not null");
        modal.value = product as HotelDetailModelCalass;
        update();
      } else {
        isEmptyData(true);
        update();
