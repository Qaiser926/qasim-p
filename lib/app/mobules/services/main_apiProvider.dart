...
class ApiProvider {
  ...
  final String url =
      ...
      final String endPart='&lang=en&currency=usd';
...
final String endurl="&lang=en&currency=usd";

    var response = await http.get(
      Uri.parse(
          // 'http://phptravels.net/api/api/main/app?appKey=phptravels&lang=en&currency=usd'),
          FlutterConfig.get("API_BASE_URL")+'api/main/app?appKey='+FlutterConfig.get("phptravels")+endurl)
      // headers: headers,
    );
    // print(response.body);
    ...
    try {
      ...
      if (response.body == 200) {
        ...
        Get.snackbar('error', 'Oops connection error');
